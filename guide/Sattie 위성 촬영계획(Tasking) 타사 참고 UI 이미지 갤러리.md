...
- 프로젝트명: viper
- 버전: v0.4 (실사용 입력폼/디버그 분리/사용자 컨텍스트 반영)
- 기준일: 2026-02-27
...
- 사용자 컨텍스트: 상단에서 `역할(as)` + `사용자(user)`를 선택하며, 변경 시 역할 기본 대시보드로 이동
- 디버그 정책: 검증 포인트/상태전이 초기화/역할 전환/테스트 시나리오는 `Debug ON`일 때만 노출
...
- 실구현 반영: 기본 모드는 실사용 입력폼(요청명/우선순위/센서/AOI/시간/제약/산출물) 중심, 테스트 시나리오 선택은 Debug 전용
- 실행 액션: `검증 -> 견적/비용 확인 -> 촬영요청 제출 -> 주문/작전 모니터링 이동`
- 검증/규칙
